...
library(plyr)
joineddata3$Gender = revalue(joineddata3$Gender, c("1"="F", "2"="M"))

#Renaming the factors of Diagnosis
joineddata3$Diagnosis = revalue(joineddata3$Diagnosis, c("A"="ASD", "B"="TD"))
```
...
#Creating variable of children with MLU over 2.7
...
#Find ID of those who did not complete all trials
missed = data %>% group_by(ID) %>% tally() %>% filter(n < 6)
missed
```
...
subs2 = select(data, ID, Visit, Diagnosis, CHI_MLU, Diagnosis, tokens_CHI )
#nothing happens if you include the same variable multiple times? 
