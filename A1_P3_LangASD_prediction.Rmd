...
return(c('RMSE' = rmselist, 'Mean RMSE' = mean(unlist(rmselist)), r2list,  'Mean R2c' =  mean(r2list$R2c), 'Mean R2m' = mean(r2list$R2m)))
}
```

```{r}
m1 = "CHI_MLU ~ Visit + Diagnosis + (1+Visit|ID)"
crossvalidate(trainingdatafold, nfolds, m1, 'CHI_MLU')

#### IT WORKS - ####

r2ListToDF(r2list)
mean(unlist(rmselist))
...
for (i in 1:nfolds) {
  train = trainingdatafold[trainingdatafold$.folds != i, ]          #set the training set
  validation = trainingdatafold[trainingdatafold$.folds == i,]      #set the validation set
  newlm = lmer(CHI_MLU ~ Visit + MOT_MLU + verbalIQ + (1+Visit|ID), train, REML = F) #creating the lmer
  rmselist[i] = Metrics::rmse(validation$CHI_MLU, predict(newlm, validation, allow.new.levels = T))  #saving model rmse
  r2list[i] = as.data.frame(r.squaredGLMM(newlm))                   #saving r2c and r2m
}
rmselist
...
for (i in 1:nfolds) {
  train = trainingdatafold[trainingdatafold$.folds != i, ]          #set the training set
  validation = trainingdatafold[trainingdatafold$.folds == i,]      #set the validation set
  newlm = lmer(CHI_MLU ~ Visit + Diagnosis + types_CHI + verbalIQ + (1+Visit|ID), train, REML = F) #creating the lmer
  rmselist[i] = Metrics::rmse(validation$CHI_MLU, predict(newlm, validation, allow.new.levels = T))  #saving model rmse
  r2list[i] = as.data.frame(r.squaredGLMM(newlm))                   #saving r2c and r2m
}
rmselist
...
#Visit, ADOS, types_CHI and verbalIQ as predictors
rmselist = list()
r2list = list()
fixedef = list()
...
for (i in 1:nfolds) {
  train = trainingdatafold[trainingdatafold$.folds != i, ]          #set the training set
  validation = trainingdatafold[trainingdatafold$.folds == i,]      #set the validation set
  newlm = lmer(CHI_MLU ~ Visit + types_CHI + verbalIQ + (1+Visit|ID), train, REML = F) #creating the lmer
  rmselist[i] = Metrics::rmse(validation$CHI_MLU, predict(newlm, validation, allow.new.levels = T))  #saving model rmse
  r2list[i] = as.data.frame(r.squaredGLMM(newlm))                   #saving r2c and r2m
  fixedef[i] = as.data.frame(fixef(newlm))                          #just a test
}
rmselist
mean(unlist(rmselist))
#mean rmse = 0.472
...
#just for fun
fixedef = as.data.frame(t(as.data.frame(fixedef)))
colnames(fixedef) = c("Intercept", "Visit", "types_CHI", "verbalIQ")
rownames(fixedef) = seq(1:4)
fixedef = as.data.frame(fixedef)
fixedef
```
